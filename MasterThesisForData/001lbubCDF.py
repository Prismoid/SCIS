'''
--- No Delay 99% のブロックが入る時間と実際に入っているブロック数 ---
3867.431640625[sec]: 89.95 %

--- Worst Delay 99% のブロックが入る時間と実際に入っているブロック数 ---
4678.85264698[sec]: 99.95 %
'''
...
### File Open ###
data = np.loadtxt('./Final/z001tsData4600000-5100000.dat')
uncleNum = np.loadtxt('./Final/z001uncleData4600000-5100000.dat')
meanLamb = (data[500000] - data[0]) / 500000
dataPFork = uncleNum / 500000

print("平均ブロック生成時間: " + str((meanLamb / (1 + dataPFork))))
...
yPlotProb = np.append(yPlotProb, [0.90, 0.995, 0.9995])
print(yPlotProb)
...
for i in range(iterate): 
    ...
    ySort = np.sort(y)
    xPlotTime[i] = [ySort[int(MCN * 0.005)], ySort[int(MCN * 0.01)], ySort[int(MCN * 0.025)], 
                    ...
                    ySort[int(MCN * 0.9)],ySort[int(MCN * 0.99)], ySort[int(MCN * 0.995)]]
'''
...
print("\n--- ETC Data 90% のブロックが入る時間と実際に入っているブロック数 ---")
xData = np.sort(xN)
print(str(xData[1800]) + "[sec]: " + str(100 * numInInterval / len(xN)) + " %")

print("\n--- Worst Delay 90% のブロックが入る時間と実際に入っているブロック数 ---")
numInInterval = sum(tmpX < mean[16] for tmpX in xN) # tmpX > timeForCalc and
print(str(mean[16]) + "[sec]: " + str(100 * numInInterval / len(xN)) + " %")

...
result = [scipy.spatial.distance.cosine(data_prob[0], data_prob[i]) for i in range(1, len(data_prob))]
bar(range(len(result)), result, 0.8, align='center', color='grcmyk')
xticks([i for i in range(len(result))], 'sub friend1 friend2 friend3 friend4 friend5'.split())
show()
